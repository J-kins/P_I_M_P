<?php
/**
 * P.I.M.P Navigation Components
 * Comprehensive navigation system for business repository platform
 */
...
class Navigation
{
     ...
     */
    public static function navigationBar(array $params = []): string
    {
        $items = $params['items'] ?? [];
     ...
     * @return string HTML output
     */
    public static function breadcrumbs(array $paths, string $separator = '/', string $class = ''): string
    {
        ob_start(); ?>
            ...
            <ol class="breadcrumbs-list">
                <?php foreach ($paths as $index => $path): ?>
                    <li class="breadcrumbs-item">
                        ...
                        <?php else: ?>
                            <a href="<?= self::url($path['url']) ?>" class="breadcrumbs-link">
                                <?= htmlspecialchars($path['label']) ?>
                            </a>
     ...
     * @return string HTML output
     */
    public static function tabbedNav(array $tabs, string $tabsId = 'tabs', string $class = ''): string
    {
     ...
     * @return string HTML output
     */
    public static function mobileNav(array $params = []): string
    {
        $items = $params['items'] ?? [];
        $activeUrl = $params['activeUrl'] ?? '';
     ...
     * @return string HTML output
     */
    public static function sidebarNav(array $params = []): string
    {
        $items = $params['items'] ?? [];
        $activeUrl = $params['activeUrl'] ?? '';
        ...
        <aside class="sidebar-nav sidebar-theme-<?= htmlspecialchars($theme) ?> <?= $collapsed ? 'collapsed' : '' ?>">
            ...
            <div class="sidebar-header">
                <div class="sidebar-logo">
                    ...
                    <img src="<?= self::asset_url($logo) ?>" alt="Logo" class="logo-image">
                    <?php else: ?>
            ...
            <nav class="sidebar-menu">
                <ul class="sidebar-list">
                    ...
                    <li class="sidebar-item 
                        ...
                        <a href="<?= self::url($item['url']) ?>" class="sidebar-link">
                            <?php if (!empty($item['icon'])): ?>
                        ...
                        <ul class="sidebar-submenu">
                            ...
                            <li class="sidebar-submenu-item <?= ($child['url'] === $activeUrl || ($child['active'] ?? false)) ? 'active' : '' ?>">
                                <a href="<?= self::url($child['url']) ?>" class="sidebar-submenu-link">
                                    <?= htmlspecialchars($child['label']) ?>
                                </a>
                            </li>
                            <?php endforeach; ?>
     ...
     * @return string HTML output
     */
    public static function socialLinks(array $links, string $layout = 'horizontal', string $theme = 'light'): string
    {
        ob_start(); ?>
     ...
     * @return string HTML output
     */
    public static function searchForm(array $params = []): string
    {
        ...
        <div class="search-form search-<?= htmlspecialchars($layout) ?>">
            <form action="<?= self::url($action) ?>" method="get" role="search">
    ...
    {
        // This should integrate with your PIMP routing system
        if (function_exists('url')) {
            return url($path);
        }
        
        // Fallback basic URL generation
        return $path;
    }

    /**
     * Asset URL helper method
     * 
    ...
    {
        // This should integrate with your PIMP asset management
        if (function_exists('asset_url')) {
            return asset_url($path);
        }
        
        // Fallback basic asset URL generation
        return $path;
    }
}
